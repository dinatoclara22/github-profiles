<script lang="ts">
	...
	import { buscaRepositorios, buscaUsuario } from "../requisições";
	...
	async function aoSubmeter() {
		const respotaUsuario = await buscaUsuario(valorInput);
		const respostaRepositorios = await buscaRepositorios(valorInput);

		if (respotaUsuario.ok && respostaRepositorios.ok) {
			const dadosUsuario = await respotaUsuario.json();
			...
			statusDeErro = respotaUsuario.status;
